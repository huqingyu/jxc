        ...
        <tr>
          ...
          <table width="100%" border="0" cellpadding="0" cellspacing="0" background="Images/pic16.gif" style="border:1px solid #CCCCCC;">
            <tr>
              ...
              <td style="font-size:14px; font-weight:bold;">搜索<font color="#FF6600">"<%=trim(xlmc)%>"</font></td>
              </tr>
          </table>
...
<%
sql="select * from xianlu where (x_name like '%"&xlmc&"%' or x_jingdian like '%"&xlmc&"%')"
select case xllx
	case "1"
		sql=sql & " and x_leixun='精品线'"
	case "2"
		sql=sql & " and x_leixun='常规线'"
end select
...
%>
                          <tr>
                            <td height="32" align="left" style="border-bottom:1px dotted #CCECB6"><a href="Xl/Xl_<%=rs("id")%>.html" target="_blank"> <span style="font-size:14px;">·<%=left(rs("x_name"),34)%></span></a> <font color="#0066FF">[<%=trim(rs("x_leixun"))%>]</font></td>
                          </tr>
<%
         rs.movenext
...
<table width="95%" border="0" cellpadding="0" cellspacing="0">
	...
	if page<=1 then
	  ...
	  Response.Write("<a href='?xlmc="&xlmc&"&xllx="&xllx&"&Page=1' class='list'>首页</a>&nbsp;&nbsp;")
	  Response.Write("<a href='?xlmc="&xlmc&"&xllx="&xllx&"&Page=" & (Page-1) & "' class='list'>上一页</a>")
    ...
    if page>=Rs.PageCount then
		...
		Response.Write("&nbsp;&nbsp;尾页")
  		...
  		Response.Write("<a href='?xlmc="&xlmc&"&xllx="&xllx&"&Page=" & (Page+1) & "' class='list'>下一页</a>")
  		Response.Write("&nbsp;&nbsp;<a href='?xlmc="&xlmc&"&xllx="&xllx&"&Page=" & rs.pagecount & "' class='list'>尾页</a>")
    end if
