...
%>
  <tr align="center">
    <td height="24" bgcolor="#FFF"><a href="admin_yddel.asp?action=look&id=<%=rs("id")%>" title="查看此预订详细信息">详情</a> <%=rs("m_name")%></td>
    <td bgcolor="#FFF"><%=rs("m_yudingtime")%></td>
    <td bgcolor="#FFF"><%=rs("m_phone")%></td>
    <td bgcolor="#FFF"><%=rs("m_renshuo")%></td>
    <td bgcolor="#FFF"><a href="../Xl/Xl_<%=rs("m_id")%>.html" title="<%=rs("m_xianlu_name")%>" target=_blank>查看线路</a></td>
  </tr>
<%
rs.movenext
wend
