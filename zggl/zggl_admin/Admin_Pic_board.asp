...
sql="insert into Picboard (board,xid,bio) values('"&t1&"','"&t3&"','"&t2&"')"
conn.execute(sql)
response.write("<script>alert('添加成功！按确定返回！');location='admin_Pic_board.asp?action=add';</script>")
end sub
sub del
...
set rs=conn.execute("select id,Leixing from Pic where Leixing="&id)
if not rs.eof then
rs.close
response.write("<script>alert('被删除的类别下面有图片，无法删除！\r\r要删除此类别请先删除属于此类别的图片！');history.go(-1);</script>")
else
conn.execute ("delete from Picboard where id="&id)
response.write("<script>alert('此类删除成功！按确定返回！');location='admin_Pic_board.asp?action=del';</script>")
end if
end sub
...
sql="update Picboard set board='"&board&"',bio='"&bio&"',xid="&xid&" where id="&id
conn.execute(sql)
response.write("<script>alert('此次修改操作成功！按确定返回！');location='admin_Pic_board.asp?action=edit';</script>")
end if
end sub
