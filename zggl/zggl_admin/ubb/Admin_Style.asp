...
Sub ShowStyleList()
	...
	Dim sManage
	sSql = "select * from xWebEditor_style"
...
Sub DoCopy()
	...
	Do While b = False
		...
		sSql = "select s_id from xWebEditor_style where s_name='" & sNewName & "'"
		oRs.Open sSql, oConn, 0, 1
	...
	Loop
	' 插入一与标准样式相同的记录，名不同
	sSql = "insert into xWebEditor_style(s_name,s_dir,s_css,s_uploaddir,s_width,s_height,s_memo,s_issys,s_fileext,s_flashext,s_imageext,s_mediaext,s_filesize,s_flashsize,s_imagesize,s_mediasize,s_stateflag,s_detectfromword,s_initmode,s_baseurl) select '" & sNewName & "',s_dir,s_css,s_uploaddir,s_width,s_height,s_memo,0,s_fileext,s_flashext,s_imageext,s_mediaext,s_filesize,s_flashsize,s_imagesize,s_mediasize,s_stateflag,s_detectfromword,s_initmode,s_baseurl from xWebEditor_style where s_id=" & sStyleID
	oConn.Execute sSql
	' 取新样式的ID
	sSql = "select s_id from xWebEditor_style where s_name='" & sNewName & "'"
	oRs.Open sSql, oConn, 0, 1
	...
	oRs.Close
	' 拷贝工具栏
	sSql = "insert into xWebEditor_toolbar(s_id,t_name,t_order,t_button) select " & sNewID & ",t_name,t_order,t_button from xWebEditor_toolbar where s_id=" & sStyleID
	oConn.Execute sSql
End Sub
...
Sub InitStyle()
	...
	If IsNumeric(sStyleID) = True Then
		sSql = "select * from xWebEditor_style where s_id=" & sStyleID
		oRs.Open sSql, oConn, 0, 1
		If Not oRs.Eof Then
...
Sub DoStyleAddSave()
	...
	sSql = "select * from xWebEditor_style where s_id=0"
...
Sub DoStyleSetSave()
	...
	If IsNumeric(sStyleID) = True Then
		' 是否存在同名
		sSql = "select * from xWebEditor_style where s_name='" & sStyleName & "' and s_id<>" & sStyleID
		oRs.Open sSql, oConn, 0, 1
		If Not oRs.Eof Then
			Go_Error "此样式名已经存在，请用另一个样式名！"
		End If
...
Sub DoStyleDel()
	If nStyleIsSys = 0 Then
		sSql = "delete from xWebEditor_style where s_id=" & sStyleID
		oConn.Execute sSql
		sSql = "delete from xWebEditor_toolbar where s_id=" & sStyleID
		oConn.Execute sSql
	End If
End Sub
...
Sub ShowStylePreview()
	Response.Write "<html><head>" & _
		...
		"<iframe ID='xWebEditor1' src='xWebEditor.asp?id=content1&style=" & sStyleName & "' frameborder=0 scrolling=no width='" & sStyleWidth & "' HEIGHT='" & sStyleHeight & "'></iframe>" & _
		"</body></html>"
End Sub
...
Sub ShowStyleCode()
	...
	Response.Write "<textarea rows=5 cols=65 style='width:100%'><IFRAME ID=""xWebEditor1"" SRC=""xWebEditor.asp?id=XXX&style=" & sStyleName & """ FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""" & sStyleWidth & """ HEIGHT=""" & sStyleHeight & """></IFRAME></textarea><br><br>"
End Sub
...
Sub ShowToolBarList()
	...
	Else
		' 取当前最大排序号
		Dim nMaxOrder
		sSql = "select max(T_order) from xWebEditor_toolbar where s_id=" & sStyleID
		oRs.Open sSql, oConn, 0, 1
	...
	sSql = "select * from xWebEditor_toolbar where s_id=" & sStyleID & " order by t_order asc"
	oRs.Open sSql, oConn, 0, 1
	Do While Not oRs.Eof
...
Sub DoToolBarAdd()
	...
	End If
	sSql = "select * from xWebEditor_toolbar where 1=0"
	oRs.Open sSql, oConn, 1, 3
	oRs.AddNew
	oRs("s_id") = sStyleID
	oRs("t_name") = s_Name
...
Sub DoToolBarModi()
	...
	sSql = "select * from xWebEditor_toolbar where s_id=" & sStyleID
	oRs.Open sSql, oConn, 1, 3
	Do While Not oRs.Eof
...
Sub DoToolBarDel()
	...
	If IsNumeric(s_DelID) = True Then
		sSql = "delete from xWebEditor_toolbar where s_id=" & sStyleID & " and t_id=" & s_DelID
		oConn.Execute sSql
...
Sub InitToolBar()
	...
	If IsNumeric(sToolBarID) = True Then
		sSql = "select * from xWebEditor_toolbar where s_id=" & sStyleID & " and t_id=" & sToolBarID
		oRs.Open sSql, oConn, 0, 1
		If Not oRs.Eof Then
...
Sub ShowButtonList()
	...
	sSql = "select * from xWebEditor_button where b_allowselect=1 order by b_order asc"
	oRs.Open sSql, oConn, 0, 1
	i = 0
	Do While Not oRs.Eof
		i = i + 1
...
Sub DoButtonSave()
	...
	sSql = "select * from xWebEditor_toolbar where t_id=" & sToolBarID
	oRs.Open sSql, oConn, 1, 3
	If Not oRs.Eof Then
