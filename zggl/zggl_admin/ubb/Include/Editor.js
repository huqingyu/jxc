...
// 附加submit提交事件,大表单数据提交,保存xWebEditor中的内容
...
function AttachReset() {
	//if (!bEditMode) setMode('EDIT');
	if(bEditMode){
		xWebEditor.document.body.innerHTML = ContentLoad.value;
	}else{
		xWebEditor.document.body.innerText = ContentLoad.value;
	}
}
...
function onPaste() {
	...
	if (sCurrMode=="EDIT"){
		...
		xWebEditor.document.selection.createRange().pasteHTML(HTMLEncode( clipboardData.getData("Text"))) ;
		return false;
	}
...
function onKeyPress(event){
	...
	if (event.keyCode==13){
		var sel = xWebEditor.document.selection.createRange();
		sel.pasteHTML("<BR>");
		event.cancelBubble = true;
		event.returnValue = false;
		sel.select();
...
function GetClipboardHTML() {
	var oDiv = document.getElementById("xWebEditor_Temp_HTML")
	oDiv.innerHTML = "" ;
...
function insertHTML(html) {
	...
	if (xWebEditor.document.selection.type.toLowerCase() != "none"){
		xWebEditor.document.selection.clear() ;
	}
	if (sCurrMode!="EDIT"){
		html=HTMLEncode(html);
	}
	xWebEditor.document.selection.createRange().pasteHTML(html) ; 
}
...
function setHTML(html) {
	...
	if(sCurrMode=="EDIT"){
		xWebEditor.document.body.innerHTML = html;
	}else{
		xWebEditor.document.body.innerText = html;
	}
}
...
function getHTML() {
	var html;
	if((sCurrMode=="EDIT")||(sCurrMode=="VIEW")){
		html = xWebEditor.document.body.innerHTML;
	}else{
		html = xWebEditor.document.body.innerText;
	}
	if (sCurrMode!="TEXT"){
		if (config.BaseUrl){
...
function format(what,opt) {
	...
	if (opt==null) xWebEditor.document.execCommand(what);
	else xWebEditor.document.execCommand(what,"",opt);
...
function setMode(NewMode){
	if (NewMode!=sCurrMode){
		...
		case "CODE":
			if (NewMode=="TEXT"){
				xWebEditor_Temp_HTML.innerHTML = xWebEditor.document.body.innerText;
				sBody = xWebEditor_Temp_HTML.innerText;
			}else{
				sBody = xWebEditor.document.body.innerText;
			}
			break;
		case "TEXT":
			sBody = xWebEditor.document.body.innerText;
		...
		case "VIEW":
			if (NewMode=="TEXT"){
				...
				sBody = xWebEditor.document.body.innerHTML;
			}
			break;
		default:
		...
		// 换图片
		try{
			document.all["xWebEditor_CODE"].className = "StatusBarBtnOff";
			document.all["xWebEditor_EDIT"].className = "StatusBarBtnOff";
			document.all["xWebEditor_TEXT"].className = "StatusBarBtnOff";
			document.all["xWebEditor_VIEW"].className = "StatusBarBtnOff";
			document.all["xWebEditor_"+NewMode].className = "StatusBarBtnOn";
			}
		catch(e){
			}
		// 换内容
		switch (NewMode){
		case "CODE":
			xWebEditor.document.designMode="On";
			xWebEditor.document.open();
			xWebEditor.document.write(config.StyleEditorHeader);
			xWebEditor.document.body.innerText=sBody;
			xWebEditor.document.body.contentEditable="true";
			xWebEditor.document.close();
			bEditMode=false;
			break;
		case "EDIT":
			xWebEditor.document.designMode="On";
			xWebEditor.document.open();
			xWebEditor.document.write(config.StyleEditorHeader+sBody);
			xWebEditor.document.body.contentEditable="true";
			xWebEditor.document.execCommand("2D-Position",true,true);
			xWebEditor.document.execCommand("MultipleSelection", true, true);
			xWebEditor.document.execCommand("LiveResize", true, true);
			xWebEditor.document.close();
		...
		case "TEXT":
			...
			break;
		case "VIEW":
			xWebEditor.document.designMode="off";
			xWebEditor.document.open();
			xWebEditor.document.write(config.StyleEditorHeader+sBody);
			xWebEditor.document.body.contentEditable="false";
			xWebEditor.document.close();
			bEditMode=false;
			break;
		}
		...
		disableChildren(xWebEditor_Toolbar);

		xWebEditor.document.body.onpaste = onPaste ;
		xWebEditor.document.onkeypress = new Function("return onKeyPress(xWebEditor.event);");
		xWebEditor.document.oncontextmenu=new Function("return showContextMenu(xWebEditor.event);");
...
function createLink(){
	...
	if (xWebEditor.document.selection.type == "Control") {
		var oControlRange = xWebEditor.document.selection.createRange();
...
function showBorders() {
	...
	var allForms = xWebEditor.document.body.getElementsByTagName("FORM");
	var allInputs = xWebEditor.document.body.getElementsByTagName("INPUT");
	var allTables = xWebEditor.document.body.getElementsByTagName("TABLE");
	var allLinks = xWebEditor.document.body.getElementsByTagName("A");
...
function doZoom(size) {
	xWebEditor.document.body.runtimeStyle.zoom = size + "%";
	nCurrZoomSize = size;
}
...
function absolutePosition(){
	...
	var RangeType		= xWebEditor.document.selection.type;
	if (RangeType != "Control") return;
	var selectedRange	= xWebEditor.document.selection.createRange();
	...
	xWebEditor.content = false;
	xWebEditor.setActive();
}
